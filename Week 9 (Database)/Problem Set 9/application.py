...
def buy():
    ...
    if request.method == "POST":
        ...
        if cash < total_price:
            return apology("You don't have enough cash to do this action!")
        else:
...
def register():
    if (request.method == "POST"):
        ...
        if not username:
            ...
            return apology('Confirmed password is required')
...
def sell():
    ...
    if request.method == "POST":
        ...
        if shares_owned < shares:
            return apology("You don't have enough shares to do this action!")

        current_cash = db.execute("SELECT cash FROM users WHERE is = ?", user_id)[0]["cash"]
